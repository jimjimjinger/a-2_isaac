...
| YOLO class id | name | 실제 시각 | mesh 형태 | USD prim prefix | USD 파일 |
|:-:|---|---|---|---|---|
| **0** | `blue_mineral`   | 밝은 cyan/teal | 수정 결정 클러스터 (비정형 polytope) | `blue_mineral_*`   | `blue_mineral.usd` |
| **1** | `yellow_mineral` | 밝은 yellow    | spike 모양 수정 결정 클러스터        | `yellow_mineral_*` | `yellow_mineral.usd` |
| **2** | `green_gas`      | 진녹색         | 정육면체 박스 (gas container)         | `green_gas_*`      | `green_gas.usd` |

> ✅ 명명 통일 (2026-05-25) — YOLO model.names / meta.json type / USD prim prefix / USD 파일명 / contract class_name 모두 동일. 이전 잔재 `mineral_red`, `red_*`, `mineral_blue.usd` 등은 모두 제거됨.
