...
class WheelOdometryNode(Node):
    ...
    역할:
    - /rover/wheel_states에서 wheel joint angular velocity를 읽는다.
    ...
    def __init__(self):
        ...
        # Parameters
        # -----------------------------
        self.declare_parameter("joint_state_topic", "/rover/wheel_states")
        ...
        # Names are based on /rover/wheel_states from joint_state_splitter_node.
        ...
        self.declare_parameter("min_steer_linear_speed", 1.0e-3)
        ...
        self.min_steer_linear_speed = (
            self.get_parameter("min_steer_linear_speed")
            .get_parameter_value()
            .double_value
        )
    ...
    def joint_state_callback(self, msg: JointState) -> None:
        ...
        use_steer_model = (
            steer_angle is not None
            and abs(steer_angle) > self.steer_deadband
            and abs(linear_v) > self.min_steer_linear_speed
        )
        ...
        if use_steer_model:
            ...
            # point-turn, 저속 회전, 또는 steering joint position이 없을 때.
        ...
        for joint_name in target_joint_names:
            if joint_name not in name_to_index:
                if joint_name not in self.warned_missing_joints:
                    self.get_logger().warn(
                        f"Wheel joint '{joint_name}' not found in "
    ...
    def get_average_joint_position(
        ...
        for joint_name in target_joint_names:
            if joint_name not in name_to_index:
                if joint_name not in self.warned_missing_joints:
                    self.get_logger().warn(
                        f"Steering joint '{joint_name}' not found in "
                        f"{self.joint_state_topic}."
                    )
                    self.warned_missing_joints.add(joint_name)
                continue
