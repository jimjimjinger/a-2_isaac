...
generated_terrains/
├── terrain_00001/
│   ├── terrain_only.usd        # 지형 메쉬 (Isaac Sim load)
│   ├── rocks_merged.usd        # 암석 메쉬 (Isaac Sim load)
...
### meta.json 전체 예시 (dummy data)

전체 스키마: [`terrain_meta_schema.json`](terrain_meta_schema.json) (JSON Schema 7).
이미 만들어진 예시: [`example_terrain_meta.json`](example_terrain_meta.json).
...
```json
{
  ...
  "spawn_locations": [
    ...
    {"x": -3.2, "y": 4.1, "z": 0.18, "yaw": 0.0, "group": "default"}
  ],
  ...
  "minerals": [
    {"id": 1, "type": "blue",   "position": {"x": 8.0, "y": 4.0, "z": 0.10}, "value": 10},
    {"id": 2, "type": "red",    "position": {"x": -5.2, "y": 7.3, "z": 0.10}, "value": 25},
    {"id": 3, "type": "yellow", "position": {"x": 14.9, "y": 14.2, "z": 0.10}, "value": 50}
  ],
...
int32 mineral_id                 # T1 meta.minerals[].id 매칭 (-1=매칭실패)
...
int32 mineral_id                 # 잡을 광물 (T2 detection의 mineral_id 또는 T1 meta의 id)
geometry_msgs/Point target_position  # 광물 좌표 (월드, m)
float32 timeout_s                # 기본 30.0
int32 request_id                 # T3 발급 unique seq (1부터)
```
