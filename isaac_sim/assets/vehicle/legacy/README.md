...
vehicle_v3.usd 정착 이전의 단계별 자산. 시연 main path 에는 사용 안 함. 발표 talking point (단계별 진화) + 회귀 진단용으로 보존 (2026-05-26 cleanup 시점).
...
> ⚠️ **AAU Mars rover 자산 (`../../rover/Mars_Rover.usd` + `SubUSDs/`) 은 위치 보존**.
> vehicle_v3.usd 가 그 안의 MDL 머티리얼 (`rover/SubUSDs/materials/*.mdl`) 을
> absolute path 로 참조 — 이동 시 차체 머티리얼 로드 실패 → rover 가
> unshaded 빨간색으로 렌더링됨 (2026-05-26 시연 회귀로 확인). 이동 금지.
