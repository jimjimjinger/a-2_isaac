#!/usr/bin/env python3
"""
mars_terrain_generator_v3.py — I1 규약 준수 화성 지형 생성기 (v3).
...
def build_meta(terrain_id, seed, minerals, spawns, difficulty):
    return {
        ...
        "generation_params": {
            "terrain": {
                "method": "base_relief+craters+hills+ridges",   # v3 — T1 엔진 흡수
...
def _remap_path_prefix(value: str, old_prefix: str, new_prefix: str) -> str:
...
def _rewrite_vehicle_paths(stage, root_path: str, old_prefix: str, new_prefix: str):
    """Referenced vehicle subtree 안의 path-like string/relationship targets를 재작성."""
    ...
    for prim in Usd.PrimRange(root):
        for attr in prim.GetAttributes():
            ...
            if not isinstance(value, str):
                continue
            new_value = _remap_path_prefix(value, old_prefix, new_prefix)
            if new_value != value:
                attr.Set(new_value)
                rewritten += 1
        for rel in prim.GetRelationships():
            ...
            for target in targets:
                path_str = target.pathString
                new_path_str = _remap_path_prefix(path_str, old_prefix, new_prefix)
...
def _attach_vehicle_reference(stage, vehicle_usd: Path, vehicle_path="/World/Vehicle",
                              translate=(0.0, 0.0, 1.0),
                              rotate_xyz=(0.0, 0.0, 0.0),
                              scale=(1.0, 1.0, 1.0),
                              base_dir: Optional[Path] = None):
    """/World/Vehicle prim을 만들고 vehicle_v1.usd를 reference 한다."""
    if not vehicle_usd.exists():
        raise FileNotFoundError(vehicle_usd)

    vehicle_prim = stage.GetPrimAtPath(vehicle_path)
    if not vehicle_prim.IsValid():
        vehicle_prim = UsdGeom.Xform.Define(stage, vehicle_path).GetPrim()

    # idempotent: 재실행 시 기존 reference와 xform stack을 정리하고 다시 author.
    vehicle_prim.GetReferences().ClearReferences()
    ...
    for op_attr in ("xformOp:translate", "xformOp:orient", "xformOp:scale",
        ...
        if vehicle_prim.GetAttribute(op_attr).IsValid():
            vehicle_prim.RemoveProperty(op_attr)

    vehicle_ref = Sdf.Reference(_asset_ref(vehicle_usd, base_dir), "/Root/Vehicle")
    vehicle_prim.GetReferences().AddReference(vehicle_ref)

    xform.AddTranslateOp().Set(
        Gf.Vec3f(*[float(v) for v in translate]))
    xform.AddRotateXYZOp().Set(
        Gf.Vec3f(*[float(v) for v in rotate_xyz]))
    xform.AddScaleOp().Set(
        Gf.Vec3f(*[float(v) for v in scale]))

    rewritten = _rewrite_vehicle_paths(
        stage,
        vehicle_path,
        old_prefix="/Root/Vehicle",
        new_prefix=vehicle_path,
    )
    return vehicle_prim, rewritten
...
def compose_world(world_path, terrain_usd, rocks_usd, marker_dir, minerals,
                  generated_at, basecamp=None, vehicle_usd: Optional[Path] = None):
    """master scene — terrain/rocks/basecamp/vehicle/광물 마커를 reference로 묶고 조명 추가."""
    require_pxr()
    ...
    if vehicle_usd is not None:
        vehicle_prim, rewritten = _attach_vehicle_reference(
            stage,
            vehicle_usd,
        ...
        print(f"[usd] vehicle attached → {vehicle_prim.GetPath().pathString}"
              f" (path rewrites: {rewritten})")
...
def maybe_export_usd(terrain_dir, terrain_id, hm, xs, ys, rocks,
    ...
    try:
        ...
        compose_world(per_terrain_world, terrain_usd, rocks_usd,
                      ...
                      vehicle_usd=VEHICLE_USD if VEHICLE_USD.exists() else None)
